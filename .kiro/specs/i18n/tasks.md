...
- [x] 1. 语言包与类型基础
- [x] 1.1 创建 `packages/locale/types.ts`（`Locale` 类型 + `LanguagePack` 契约）
...
- [x] 1.2 (P) 创建 `packages/locale/lang/zh-cn.ts`
...
- [x] 1.3 (P) 创建 `packages/locale/lang/en.ts`
...
- [x] 2. locale 注册入口
- [x] 2.1 创建 `packages/locale/index.ts`（i18n 单例 + 默认语言 + 公开导出）
...
- [x] 3. useLocale 切换入口
- [x] 3.1 创建 `packages/hooks/useLocale.ts`（`t` / `locale` / `setLocale`）
...
- [x] 3.2 创建 `packages/hooks/index.ts`（hooks barrel，re-export useLocale）
...
- [x] 4. 集成与验证
- [x] 4.1 验证 `./locale` 与 `./hooks` exports 映射由 foundation 暴露
